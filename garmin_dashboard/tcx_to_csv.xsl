<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="g:Activity">Index,Time,Position Lat,Position Lon,Heart Rate,Distance,Altitude,Lap<xsl:text>&#xa;</xsl:text>
    <xsl:for-each select="g:Lap">
      ...
      <xsl:for-each select="g:Track/g:Trackpoint">
        ...
        <xsl:value-of select="position()"/>,<xsl:value-of select="g:Time"/>,<xsl:value-of select="g:Position/g:LatitudeDegrees"/>,<xsl:value-of select="g:Position/g:LongitudeDegrees"/>,<xsl:value-of select="g:HeartRateBpm/g:Value" />,<xsl:value-of select="g:DistanceMeters" />,<xsl:value-of select="g:AltitudeMeters" />,<xsl:value-of select="$LapNo"/><xsl:text>&#xa;</xsl:text>
